...
import { UserPlus, CheckCircle, AlertTriangle, GraduationCap, Search, School, ShieldCheck } from 'lucide-react';
        ...
        <div className="relative max-w-[1000px] mx-auto w-full px-4 py-8 sm:py-10 grid lg:grid-cols-[1fr_460px] gap-6 lg:gap-8 items-start lg:items-center">
          ...
          <div className="text-white text-center lg:text-left">
            <span className="inline-flex items-center gap-2 bg-white/10 backdrop-blur border border-white/10 rounded-full px-3 py-1.5 text-[11px] font-semibold text-accent mb-4">
              <ShieldCheck size={12} /> One code, instant enrollment
            </span>
            <span className="hidden lg:flex w-12 h-12 rounded-xl bg-white/10 items-center justify-center mb-5"><UserPlus size={24} className="text-accent" /></span>
            <h1 className="text-white text-[28px] sm:text-[32px] lg:text-[34px] font-bold leading-[1.05] tracking-tight">
              Enroll in<br className="hidden sm:block" /> a Class
            </h1>
          ...
          <div className="flex items-center gap-2.5 mb-1.5">
            <span className="w-10 h-10 rounded-xl bg-navy-900 text-white flex items-center justify-center"><UserPlus size={20} /></span>
            <div>
              <h1 className="text-[18px] font-bold text-navy-800 leading-tight">Enroll in a Class</h1>
              <p className="text-[11px] font-semibold tracking-[.08em] uppercase text-faint">Join with a class code</p>
            </div>
          </div>
            ...
            <>
              <form onSubmit={lookup} className="flex flex-col gap-2.5 mb-6">
                <Input label="Class Code" value={codeEntry} onChange={e => setCodeEntry(e.target.value)}
                  placeholder="e.g. BSCS2A" icon={Search} className="!font-mono !uppercase !tracking-wide !bg-canvas focus:!bg-surface"
              ...
              {klass && (
                <div style={{ animation: 'fadeInUp .25s' }}>
                  <div className="border border-success/15 bg-success-bg/60 rounded-[12px] px-4 py-3.5 mb-4">
                    <div className="text-[11px] font-semibold tracking-[.08em] uppercase text-success mb-1">Class found</div>
                    ...
                    {klass.section && (
                      <Badge tone="success" className="!mt-2">You will be enrolled in section {klass.section}</Badge>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <Input label="Full Name" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Dela Cruz, Juan A." className="!bg-canvas focus:!bg-surface" />
