...
function CreateExamInner() {
  ...
  const saveExam = async () => {
    ...
    setSaving(true);
    const body = { title: title.trim(), description: desc.trim(), time_limit: timeLimit };
    try {
      if (examId) {
            ...
            {!questions.length ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {questions.map((q, i) => {
                  ...
                  return (
                    <div key={q.id} style={{
                      background: '#f5f8ff', border: '1px solid #c8d8f0', borderRadius: 10,
                      padding: '16px 18px', transition: 'box-shadow .15s',
                    }}
                      onMouseEnter={e => e.currentTarget.style.boxShadow = '0 2px 12px rgba(15,32,68,.08)'}
